...
export default function AppHeader({
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-border bg-surface/90 px-4 py-2.5 backdrop-blur">
      ...
      <div className="mt-2 flex justify-end">
        <div className="flex items-center gap-2.5 rounded-full border border-border bg-surface-2/60 py-1 pl-1.5 pr-3 text-xs font-semibold">
          <span className="flex items-center gap-1">
            {/* eslint-disable-next-line @next/next/no-img-element -- ไฟล์ static ขนาดเล็กคงที่ (128x128) ไม่ต้องผ่าน next/image optimizer ซึ่งแคชค้างเวลาสลับไฟล์ */}
            <img src="/assets/misc/coin-silver-icon.png" alt="" className="h-5 w-5 rounded-full object-cover" />
            <span className="text-silver">{silver.toLocaleString()}</span>
          </span>
          <span className="flex items-center gap-1">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/assets/misc/coin-gold-icon.png" alt="" className="h-5 w-5 rounded-full object-cover" />
            <span className="text-gold">{gold.toLocaleString()}</span>
          </span>
          <span className="flex items-center gap-1">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/assets/misc/shard-icon.png" alt="" className="h-5 w-5 object-contain" />
            <span className="text-shard">{shards.toLocaleString()}</span>
          </span>
        </div>
      </div>
    </header>

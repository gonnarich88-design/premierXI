...
| Schema แบบอ่านเร็ว | `database.dbml` (gen ด้วย `npm run db:dbml`) |
...
│   └── generate-dbml.ts                 # gen database.dbml จาก schema จริง
...
| `npm run db:dbml` | Regen `database.dbml` จาก schema จริง (รันหลังแก้ schema) |
